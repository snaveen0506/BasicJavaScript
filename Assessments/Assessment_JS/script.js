...
console.log(forEachElement(ages, call));
...
function fetchData(url, callback) {
  setTimeout(() => {
    const error = Math.random() > 0.5; //create random numbers
    const errMessage = "It' s not greater than 0.5";
    const sucMessage = "It' s greater than 0.5";

    if (error === true) {
      callback(`Error occured is '${errMessage}' and URL is '${url}'`);
    } else {
      callback(
        `The response is successful the url is '${url}' and the message is '${sucMessage}'`
      );
    }
  }, 3000); //print result after 3 sec
}

function callBack(res) {
  if (res) {
    console.log(res); //res is 174 and 177 messages
  }
}

fetchData("https://www.demo.com/", callBack);
